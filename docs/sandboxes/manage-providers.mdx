...
network policy is enabled by default. To attach provider credentials without
adding provider profile network policy layers, disable it:
...
also contribute provider-generated network policy entries unless
`providers_v2_enabled` is disabled at the gateway. When the setting is disabled,
providers only provide credentials.

<Warning>
Provider attachment changes on a running sandbox require operator approval and
are applied through the draft policy workflow.
